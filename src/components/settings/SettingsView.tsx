// Settings screen — appearance, transcription, keywords, library stats, maintenance.
...
export function SettingsView() {
  ...
  const { whisperStatus, models, activeDownloads, fetchEnvironment, fetchModels, startDownload, removeModel } = useTranscriptionStore();
  ...
  async function handleDeleteOrphaned() {
    setWorking(true); setConfirmAction(null);
    try {
  ...
  async function handleDownloadModel(name: string) {
    try { await startDownload(name); }
    catch (e) { showToast(`Download failed: ${String(e)}`, 'error'); }
  }

  async function handleDeleteModel(name: string) {
    try { await removeModel(name); showToast(`Model "${name}" deleted.`, 'success'); }
    catch (e) { showToast(String(e), 'error'); }
  }
  ...
  return (
    <div style={{ height: '100%', overflowY: 'auto' }}>
      <div style={{ maxWidth: '640px', padding: '24px 24px 48px' }}>
                ...
                {whisperStatus === null ? (
                  ...
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: '7px' }}>
                    ...
                    <div>
                      ...
                      <p style={{ fontSize: '11px', color: 'var(--text-secondary)', margin: '0 0 4px' }}>Install via Homebrew:</p>
                      ...
                      <div key={model.name} style={{
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                            ...
                            {model.installed && (
                              <span style={{ fontSize: '10px', color: 'var(--status-online)', background: 'rgba(48,209,88,0.12)', borderRadius: '3px', padding: '1px 5px' }}>Installed</span>
                            )}
                          </div>
                          ...
                          {isDownloading && (
                            <div style={{ marginTop: '6px' }}>
                              ...
                              <p style={{ fontSize: '10px', color: 'var(--text-tertiary)', margin: '3px 0 0' }}>Downloading… {downloadPct}%</p>
                            </div>
                          )}
                        </div>
                        {!isDownloading && (model.installed ? (
                          <button onClick={() => handleDeleteModel(model.name)} title="Delete model"
                            style={{ display: 'flex', alignItems: 'center', fontSize: '12px', color: 'var(--text-tertiary)', background: 'none', border: 'none', cursor: 'pointer', padding: '4px' }}
                            onMouseEnter={e => (e.currentTarget.style.color = 'var(--color-danger)')}
                            onMouseLeave={e => (e.currentTarget.style.color = 'var(--text-tertiary)')}>
                            <Trash2 size={13} />
                          </button>
                        ) : (
                          <button onClick={() => handleDownloadModel(model.name)} disabled={!whisperStatus?.found}
                            title={whisperStatus?.found ? `Download ${model.name}` : 'whisper-cli not found'}
                            style={{ display: 'flex', alignItems: 'center', fontSize: '12px', color: 'var(--color-accent)', background: 'none', border: 'none', cursor: whisperStatus?.found ? 'pointer' : 'not-allowed', padding: '4px', opacity: whisperStatus?.found ? 1 : 0.4 }}
        ...
        {/* Keyword Auto-Marking */}
        <Section label="Keyword Auto-Marking">
          <Panel>
            <KeywordSettings />
          </Panel>
        </Section>
