...
export interface AssetMetadataEntry { description: string; location: string; }
export interface FilterCounts { by_type: Record<string, number>; by_drive: Record<string, number>; orphaned: number; missing: number; has_markers: number; }

export async function getStats(): Promise<LibraryStats> { return invoke<LibraryStats>('settings_get_stats'); }
export async function getFilterCounts(): Promise<FilterCounts> { return invoke<FilterCounts>('settings_get_filter_counts'); }
...
export async function getAssetMetadata(): Promise<Record<string, AssetMetadataEntry>> { return invoke<Record<string, AssetMetadataEntry>>('settings_get_asset_metadata'); }
